buto = document.getElementsByTagName('div')[3];
function handleAddColumn() {
  let butoPlus = Array.from(document.querySelectorAll("div > div:nth-child(4) > button")).find(boton => boton.innerHTML.includes("+"));
  if (buto.innerHTML.includes('+')) {butoPlus.click(); again(butoPlus)}  
...
let newLetter = String.fromCharCode(letterMatch[0].trim().charCodeAt(0) + 1);
...
function handleAddYear() {
  ...
  const lastYear = Number(table.rows[table.rows.length - rs.getAttribute('rowspan')].cells[0].firstChild.value);
  ...
  const newYear = { year: lastYear + 1, categories: categories.map(cat => {
  ...
  newYear.categories.forEach((cat, index) => {
    ...
    cat.valores.forEach((valor, valueIndex) => {
      ...
      input.addEventListener("change", function () {
        ...
        updateValueByYearAndCategory(newYear.year, cat.category, valueIndex, this.value);
      });
  ...
  updateFormElementNames()
  if (!buto.innerHTML.includes('+')) {trRowsUpdate()};
  rowsUpdate()
  rowSpan()
  borderBottom()
}
...
let proButton = document.querySelector("div > div:nth-child(4) > button:nth-child(1)")
let sellButton = document.querySelector("div > div:nth-child(4) > button:nth-child(2)")
proButton.addEventListener('click', eliminarFilasProduccion)
sellButton.addEventListener('click', eliminarFilasVentas)

function ordo(){return filas = document.querySelectorAll("#data-table tr")}
let priceLength;

let rows = document.querySelectorAll("#data-table tr");
function rowsUpdate() {rows = document.querySelectorAll("#data-table tr")};
...
function eliminarFilasProduccion() {
  ...
  rowSpanUpdate();
  rowSpaNumber();
  sellButton.removeEventListener('click', eliminarFilasVentas)
...
function eliminarFilasVentas() {
  ...
  rowSpaNumber();
  proButton.removeEventListener('click', eliminarFilasProduccion)
