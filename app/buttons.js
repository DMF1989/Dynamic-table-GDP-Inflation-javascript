...
function highlight(x, typeLine = "none") {
  if (buto.innerHTML.includes("+") == false) {
    for (let i = x; i < trRows.length; i += 3) {
      if (x == 1) {
        g = 1;
        y = 2;
      ...
      for (let j = g; j < arreglo.length + y; j++) {
        trRows[i].childNodes[j].childNodes[0].style.textDecoration = typeLine;
        trRows[i].childNodes[j].childNodes[0].style.color = textColor;
      }
    }
  }
}
